...
def printTable(matrix, useFieldNames=False, color=None):
    ...
    finalTable = []
    # This is the matrix to sort the length of each element
    lengthMatrix = []
    ...
    for i in range(rows):
        ...
        for j in range(cols):
            ...
            # Append the current element to the lengthMatrix
            lengthMatrix.append(len(str(matrix[i][j])))
    # Sort the length matrix so that we can find the element with the longest length
    lengthMatrix.sort()
    # Store that length
    largestElementLength = lengthMatrix[-1]
    ...
    for i in range(rows):
        ...
        for j in range(cols):
            # If we are not using colors just add a space and the element that should be in that position to a variable which will store the current element to work on
            if (color == None):
                currentEl = " " + matrixToWorkOn[i][j]
            # If we are using colors then do the same thing but add color
            else:
    ...
    # If we are using field names (row that looks like +-----+-----+-----+ under the first row) then...
    if (useFieldNames):
